...
basename=${input##*/}
output=${basename%%.*}.halfsize.24fps.x264.mp4
fi
...
ffmpeg -hide_banner -i $input -vf "scale=ceil(iw/4)*2:ceil(ih/4)*2[a]; [a]fps=fps=24:round=zero:eof_action=pass" -pix_fmt yuv420p -vcodec libx264 $output
