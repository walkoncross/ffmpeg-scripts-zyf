...
cmd_name=$(basename $0)
show_usage() {
    echo "Rotate input video 90 degree clockwise and save into h264 encoded video with .mp4 format, with film tune settings"
    echo "Usage: "
    echo "    ${cmd_name} -h"
    echo "    ${cmd_name} --help"
...
# Add your ffmpeg command here to rotate the video by 90 degrees clockwise
if [[ -z "$fps" ]]; then
    # ffmpeg -i "$input" -vf "transpose=1" -c:v libx264 -preset slow -tune film -crf 22 -c:a copy "$output"
    ffmpeg -i "$input" -vf "transpose=1" -c:v libx264 -preset medium -c:a copy "$output"
else
    # ffmpeg -i "$input" -vf "transpose=1" -r "$fps" -c:v libx264 -preset slow -tune film -crf 22 -c:a copy "$output"
    ffmpeg -i "$input" -vf "transpose=1" -r "$fps" -c:v libx264 -preset medium -c:a copy "$output"
fi
