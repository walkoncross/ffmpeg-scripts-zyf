...
function show_usage {
	cmd_name=$(basename $0)
    echo "Save input video into vp9 encoded video with .webm format, with film tune settings"
    echo "Usage: "
	echo "	${cmd_name} -h"
	echo "	${cmd_name} --help"
    ...
    echo "		<output_video>: optional, path name ended with .webm for the output video"
}
