...
basename=${input##*/}
output=${basename%%.*}.resize0.25.24fps.x264.mp4
fi
...
ffmpeg -hide_banner -i $input -vf "scale=ceil(iw/8)*2:ceil(ih/8)*2[a]; [a]fps=fps=24:round=zero:eof_action=pass" -pix_fmt yuv420p -vcodec libx264 $output
