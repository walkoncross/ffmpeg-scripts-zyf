...
basename=${input##*/}
output=${basename%%.*}.50fps.x264.mp4
fi
...
# ffmpeg -hide_banner -i $input -vf "pad=ceil(iw/2)*2:ceil(ih/2)*2[a]; [a]fps=50"  -pix_fmt yuv420p -vcodec libx264 -preset fast -tune film $output
# ffmpeg -hide_banner -i $input -vf "pad=ceil(iw/2)*2:ceil(ih/2)*2[a]; [a]fps=50"  -pix_fmt yuv420p -vcodec libx264 -tune film $output
ffmpeg -hide_banner -i $input -vf "pad=ceil(iw/2)*2:ceil(ih/2)*2[a]; [a]fps=50" -pix_fmt yuv420p -vcodec libx264 $output
